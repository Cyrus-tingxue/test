...
function renderAdventureUI(container) {
    container.innerHTML = `
        <div style="display: grid; grid-template-rows: auto 1fr auto; height: calc(100vh - 140px); gap: 1rem;">
            <!-- Status Bar -->
            <div class="card" style="padding: 0.75rem 1.5rem; display: flex; justify-content: space-between; align-items: center; background: rgba(30, 41, 59, 0.8); backdrop-filter: blur(10px);">
                ...
                <div>
                     <span style="color: var(--text-muted); font-size: 0.9rem;">${adventureState.status}</span>
                     <button onclick="resetAdventure()" style="margin-left: 1rem; background: transparent; border: 1px solid var(--border); color: var(--text-muted); padding: 0.2rem 0.6rem; border-radius: 0.25rem; cursor: pointer; font-size: 0.8rem;">🔄 重开</button>
                </div>
            </div>

            <!-- Log Area -->
            <div id="adv-log" class="card" style="overflow-y: auto; padding: 1.5rem; display: flex; flex-direction: column; gap: 1.5rem; background: rgba(15, 23, 42, 0.6);">
            ...
            <div class="card" style="padding: 1rem;">
                <div id="adv-choices" style="display: flex; gap: 0.5rem; margin-bottom: 1rem; overflow-x: auto; padding-bottom: 0.5rem;">
                    <!-- Choices injected here -->
                </div>
                
                <div style="display: flex; gap: 0.5rem;">
                    <input type="text" id="adv-input" placeholder="你想做什么？(例如：向北走、检查背包、攻击史莱姆)..." 
                        style="flex: 1; padding: 1rem; border-radius: 0.5rem; border: 1px solid var(--border); background: var(--bg-darker); color: white;"
                        onkeydown="if(event.key === 'Enter') adventureAction(this.value)">
                    <button onclick="adventureAction(document.getElementById('adv-input').value)" class="primary-btn" style="width: auto; padding: 0 1.5rem;"><i data-lucide="send"></i></button>
                </div>
            </div>
        </div>
    `;
...
async function adventureAction(actionText) {
    ...
    try {
        ...
        adventureState.messages.push({ role: 'assistant', content: `[系统错误] 与平行宇宙的连接中断: ${e.message}` });
        renderAdventureUI(document.getElementById('content-area'));
    }
}
...
async function doChat() {
    ...
    if (!apiKey) {
        appendMessage('system', '⚠️ 请先在左侧配置 API Key');
        return;
    }

    // Prepare history (simplified for demo, ideally traverse DOM or keep state)
    // Here we just send the current message + system prompt for simplicity, 
    // or we could maintain a global chat state variable.
    // Let's implement a simple global state for now.
    if (!window.chatState) window.chatState = [];
    window.chatState.push({ role: 'user', content: message });

    const requestMessages = [
        { role: "system", content: "你是 Office AI Mate，一个全能的 AI 助手。请用准确、有深度、有条理的方式回答用户。如果用户用中文提问，请用中文回答。" },
        ...window.chatState
    ];
...
async function doGenerateMindMap() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Clean old
            renderDiv.removeAttribute('data-processed');
            renderDiv.innerHTML = res.code;
            ...
            import('https://cdn.jsdelivr.net/npm/mermaid@10/dist/mermaid.esm.min.mjs').then(mermaid => {
                ...
                mermaid.default.run({
                    ...
                    console.error("Mermaid auto run error:", err);
                    if (errorDiv) {
                        errorDiv.innerHTML = `<div>⚠️ 自动渲染失败：语法不标准。<br>您可以尝试在“查看源码”中手动修正。</div>`;
        ...
        <div class="card">
            ...
            <div class="guide-section">
                ...
                <div style="background: var(--bg-darker); padding: 1.5rem; border-radius: 0.75rem; border: 1px solid var(--border);">
                    ...
                    <ul style="margin-top: 0.5rem; color: var(--text-muted);">
                        <li>在手机端将聊天记录全选 -> 更多 -> 邮件发送/转发至文件传输助手，保存为文本。</li>
                        <li>或者使用第三方微信迁移工具导出格式化后的文本。</li>
                    </ul>
                </div>
            </div>
        </div>
